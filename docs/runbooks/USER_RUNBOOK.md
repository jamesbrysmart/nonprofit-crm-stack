...
- **Manual gift entry** – keyboard-first form for one-off or catch-up gifts. As you type, duplicate detection proposes existing donors; you explicitly choose “Use donor” (or clear the selection) before the summary card locks it in. Leaving the card empty creates a new donor. An optional toggle lets you link the gift to an existing recurring agreement. The form submits into staging (auto-process disabled unless you explicitly change it) so you can review the row before processing.
...
3. **Watch duplicate warnings**  
   ...
   _Known gap:_ this warning does **not** check already processed gifts yet—spot-check the donor’s recent gifts in Twenty if you suspect they were posted earlier.
4. **Link to a recurring agreement (optional)**  
   Toggle “Part of a recurring agreement” to reveal a filtered list of recent Twenty `RecurringAgreement` IDs. Select the correct agreement so downstream rollups advance automatically when the row processes.
5. **Submit and monitor staging**  
   Successful submission shows either the new gift ID (auto-processed) or a staging acknowledgement. Head to the staging queue to edit coding, resolve duplicates, and choose when to process.
...
- **On processing:** Agreement status flips to `active`, `nextExpectedAt` updates, and the posted Gift retains provider metadata (`recurringAgreementId`, `provider`, `providerPaymentId`, `providerContext`).
- **Auto-process guardrail:** If Stripe didn’t supply a confident email, the row arrives with `autoProcess=false` so you review it before posting.
...
- Use the staging queue filter by agreement ID to investigate missed payments; compare `nextExpectedAt` against the latest processed gift.
...
3. **Handle cancellations or pauses**
   - Perform the action in Stripe/GoCardless or Twenty and update the agreement status. While paused, auto-process stays disabled so accidental charges don’t post.
4. **Audit receipts & reporting**
   - Confirm processed gifts include `provider` and `recurringAgreementId` for downstream receipts.
...
- **Agreement not updating:** Processing only advances `nextExpectedAt` when the staging row carries the correct `recurringAgreementId`. Edit the drawer and reprocess if needed.
...
- **Too many manual reviews:** Enable `autoProcess` on trusted recurring agreements. Rows for those agreements skip manual processing unless validation fails.
...
- Duplicate warning in manual entry only checks staging rows; processed-gift check is planned.

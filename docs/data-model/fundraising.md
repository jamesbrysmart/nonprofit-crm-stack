...
| Processing to Gift | Immediate GraphQL call to `/gifts`. | After staging row approved, map to Gift schema (see table above). | Build processing flow in Twenty metadata: statuses like `ready_for_commit`, `committed`, `commit_failed`; log Gift ID for reconciliation & rollback. Honour `gift_batch.auto_promote` when advancing rows. |

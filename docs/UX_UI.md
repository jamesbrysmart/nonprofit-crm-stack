...
- `apps/fundraising/nonprofit-fundraising`: front components now have a clearer shared local style layer for repeated primitives such as panels, labels, text, inputs, textarea, links, code blocks, and pill treatments; new UI work should extend that layer before adding fresh inline style variants.
...
- `apps/fundraising/nonprofit-fundraising`: `Gift batch` coding should behave like a compact bulk-edit tool rather than a diagnostics panel. The preferred shape is one short purpose line plus unified row-based coding rules for `Appeal`, `Fund`, and `Appeal source`, with concise apply modes such as `Leave`, `Clear`, `Apply to all`, and `Only where blank`.

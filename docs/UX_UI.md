...
- _Prototype note (2026-03-09, assistant drawer):_ a fundraising copilot drawer was prototyped as a drawer-first surface with concise header copy, a calm chat thread, compact evidence rows, and lightweight follow-up chips. Internal AI mechanics stayed out of the primary UI so the surface read as a native review drawer with conversational affordances rather than a bespoke chat app. That prototype is not on `main`; it was preserved separately on the `services/fundraising-service` branch `feat/assistant-exploration` for later review.

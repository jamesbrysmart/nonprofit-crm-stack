...
- _As-built (2026-03-09, recurring donations):_ the recurring mode now follows the shared record-browser shell with summary band, exception-bucket filters, search, visible sort, applied-filter chips, pagination, and a read-focused agreement review drawer. The recurring list/drawer now resolve donor display from either hydrated `contactId` records or embedded contact relation data, and the review drawer includes a direct donor-record link because the primary follow-up path for missed regular donations is usually donor contact; agreement-level `Open in Twenty` remains secondary.
...
- _As-built (2026-03-06, Storybook workflow packs):_ Storybook coverage now includes control-driven `Operations/Summary Band` plus initial workflow packs for `Manual Gift Entry` and `Reconciliation` (`Workflows/*` stories) with composed views, key state scenarios, and playground controls to support UX review beyond static component snapshots. The intended use is exploratory and comparative rather than prescriptive: pattern stories shape shared UI ideas, while workflow stories show how real pages consume them.
- _As-built (2026-03-09, record-browser baseline):_ fundraising list views now converge on a shared record-browser model: summary band, filter scope, visible sort, applied-filter chips, pagination, and right-side review drawer. Storybook `Patterns/Record Browser Shell` is the current baseline candidate, with `table` and `card list` both allowed as record surfaces. The earlier `queue + drawer` framing now reads best as an operational variant within this broader family rather than a competing rule.
- _As-built (2026-03-09, app shell):_ the fundraising app now behaves as a single workspace with four primary modes (`Gift processing`, `Recurring donations`, `Appeals`, `Reconciliation`) moved into the lightweight left rail above utility actions. The main header is intentionally minimal, and utility entry points stay drawer-driven.
- _As-built (2026-03-09, assistant drawer):_ the fundraising copilot remains drawer-first and intentionally restrained: concise header copy, a calm chat thread, compact evidence rows, and lightweight follow-up chips. Internal AI mechanics stay out of the primary UI so the surface reads as a native review drawer with conversational affordances rather than a bespoke chat app. The drawer opens into a quiet briefing prompt state rather than auto-running, and the top assistant response is framed as a short `Copilot summary` with cards beneath acting as evidence.
- _As-built (2026-03-09, compact list chrome):_ the primary fundraising list views now use a tighter shell treatment: reduced summary-band padding, smaller metric/chip density, hidden search/sort labels where the surrounding context is already clear, and more restrained pagination so more records are visible above the fold.
- _As-built (2026-03-09, gift staging queue):_ gift staging now defaults to an active-work queue rather than showing processed history by default. `Processed` is opt-in via filters, row diagnostics are compressed into one primary state plus one short review summary, and the review drawer now carries a clearer `Review focus` section so detailed blockers/warnings move out of the list without losing operational context.
- _As-built (2026-03-10, gift staging density):_ the queue now uses a tighter one-line posture for default rows, with `Gift date` rendered in a more compact date-only format so rows stay denser without forcing the table wider.
- _As-built (2026-03-09, gift staging scope):_ gift staging now treats an opened gift batch as a parent-scope mode rather than just a hidden filter. Workspace mode surfaces active batches as explicit entry points, while batch scope changes the header posture, keeps batch-specific diagnostics/actions in context, and provides a clear `Back to workspace` exit.
- _As-built (2026-03-09, gift staging drawer):_ the staging drawer now uses a leaner three-tab structure: `Donor` (default), `Details`, and `Audit`. The first screenful is intentionally donor-first with a compact identity/status band and less repeated context, while secondary coding fields move into `Details` and audit/raw payload stay out of the main review flow.
- _As-built (2026-03-10, donor review):_ the gift-staging donor tab now explicitly separates the staged donor identity from CRM donor state so staff do not mistake incoming name/email for a confirmed match. `Matched`, `Review match`, and `Unmatched` states are called out directly, likely donors surface immediately with confirming details, and `Search donors` opens with an immediate prefilled search from the staged name/email.
- _As-built (2026-03-10, batch entry + actions):_ gift processing workspace now promotes active batches as a first-class `New batches (N)` entry point rather than hiding batch mode in filters or row badges. Opening that inbox reveals explicit `Open batch` choices, and batch scope now exposes `Run donor match` and `Process batch` as the two focused batch-level actions, with lightweight inline run status for each.
- _As-built (2026-03-10, manual gift entry density):_ the manual-gift drawer now uses tighter section chrome and less header copy so more of the working form lands above the fold. Section headers/cards are intentionally smaller, the recurring-association section starts collapsed until needed, and the primary submit action stays visually anchored at the bottom of the drawer.
- _As-built (2026-03-10, appeals metrics):_ the appeals view now accepts aggregate money fields returned as either `value` or `amountMicros`, so `Raised` stays in sync with the underlying appeal record instead of dropping to an em dash when Twenty returns micros-based money payloads.
- _As-built (2026-03-10, appeals shell fit):_ the Appeals workspace is now better understood as a valid record-browser variant rather than as something that failed to fit the gift-processing queue pattern. The shared layer is the operating model and shell expectations; the appeal-specific record surface can stay looser if that better supports review and maintenance of appeal context.
- _As-built (2026-03-09, households):_ `Households` is no longer a persistent workspace utility. It now launches from manual-gift context and is framed as a guided household-resolution flow (`find existing` or `create new`) instead of an always-visible mini admin app.

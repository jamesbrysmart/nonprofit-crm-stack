...
- `apps/fundraising/nonprofit-fundraising`: Opportunity payment recording should live on a dedicated `Payments` tab with a compact `Record payment` action and native linked `Gifts`, leaving the main `Funding` tab focused on application/award fields.
...
- `apps/fundraising/nonprofit-fundraising`: the `Gift` `Home` tab summary should avoid generic posture prose and instead use compact, populated-only summary strips for gift context, fundraising context, and financial/lifecycle context.
...
- `apps/fundraising/nonprofit-fundraising`: the standard `Person` record constituent-context widget should act as a compact nonprofit signal strip above native fields, not as a replacement for standard contact fields. It should show role/status chips for donor, fundraiser, Gift Aid, and recurring context, then use balanced compact metrics for donation rollups rather than making lifetime giving the only hero value. Keep the no-context state quiet.
- `apps/fundraising/nonprofit-fundraising`: the standard `Company` record constituent-context widget should follow the same compact signal-strip pattern as `Person`, using factual funding rollups and upcoming funding dates while leaving website, address, and other standard organisation fields to native fields.
- `apps/fundraising/nonprofit-fundraising`: standard `Person` and `Company` records should be extended with app-owned `definePageLayoutTab` tabs rather than replacement `definePageLayout` record pages. Use a constituent-context summary above curated grouped `FIELDS`, then place heavier relationship lists in separate native `FIELD` widgets below the grouped fields.
...
- `apps/fundraising/nonprofit-fundraising`: front components now share local UI primitives from `src/front-components/front-component-ui.tsx` for repeated panels, labels, text, inputs, textarea, action buttons, links, code blocks, badges, compact metadata, context signal strips, and divider treatments; new UI work should extend that neutral layer before adding fresh inline style variants. Use `ActionButton` for front-component actions and `CompactMetaGrid` / `CompactMetaItem` for repeated compact label/value summaries.
- `apps/fundraising/nonprofit-fundraising`: use `SummaryStrip` / `SummaryStripItem` for compact split-summary facts that would otherwise consume vertical space. This is the preferred framed summary-strip pattern for record and workflow summaries; avoid assembling the divider, grid, labels, and values manually in new components.

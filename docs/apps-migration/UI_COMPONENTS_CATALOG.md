...
| `Opportunity record payment` | Front component | Standard `opportunity` record `Payments` tab | Create a committed company Gift linked to the current Opportunity, with compact funding/payment context | Shared model candidate | Mirrors the recurring-agreement payment pattern where it fits, but keeps manual gift entry available for company gifts without an Opportunity |
...
| `Opportunity payments tab` | Native widget assembly | Standard `opportunity` record `Payments` tab | Payment action plus native linked Gifts relation widget | Shared concept | Keeps payments/gifts away from the main Funding fields tab so vertical space is not consumed on every Opportunity |

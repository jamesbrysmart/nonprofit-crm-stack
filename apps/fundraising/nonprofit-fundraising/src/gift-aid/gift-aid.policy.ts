import type { GiftAidDeclarationService } from './gift-aid.declarations';
...
export const applyGiftAidMetadata = async (
  declarationService: GiftAidDeclarationService,
  payload: GiftAidEvaluatedPayload,
  enabled: boolean,
  ...
  if (explicitDeclarationId) {
    const explicitDeclaration =
      await declarationService.getGiftAidDeclarationById(explicitDeclarationId);
  ...
  const declaration =
    await declarationService.resolveApplicableGiftAidDeclaration(withDeclaration);
